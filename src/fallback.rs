        ...
        + 'static,
{
    ...
    async fn make_request(&self, req: RequestPacket) -> Result<ResponsePacket, TransportError> {
        let transports = {
            let mut res = (*self.transports).clone();
            res.sort_by(|a, b| b.cmp(a));
            res
        };

        let res = transports.into_iter().map(|mut t| {
            let req_clone = req.clone();
            let start = Instant::now();

            async move {
                let result = t.call(req_clone).await;
                ...
                (result, t, start.elapsed())
            }
        });

        let mut res = futures::stream::iter(res)
            .buffer_unordered(self.active_transport_count)
            .boxed();

        // Wait for the first successful response or until all fail
        let mut last_error = None;

        while let Some((result, transport, duration)) = res.next().await {
            match result {
                Ok(response) => {
                    // Record success
